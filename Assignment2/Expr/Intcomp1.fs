...
let rec eval e (env : (string * int) list) : int =
    ...
    | Var x             -> lookup env x 
    | Let(lst, ebody) ->
        // basically foreach element attach str (var name) to evaluated expr.
        let env1 = List.fold (fun accEnv (str, exp) ->
                let variable = eval exp accEnv
                (str, variable) :: accEnv) env lst
...
let rec tcomp (e : expr) (cenv : string list) : texpr =
    ...
    | Let(lst, ebody) ->
        ...
        let rec helper lst accEnv =
            ...
            | [] -> tcomp ebody accEnv
            | (name, exp) :: xs ->
                let cenv1 = name :: accEnv
                let expr = tcomp exp accEnv
                let rest = helper xs cenv1
                TLet(expr, rest)
